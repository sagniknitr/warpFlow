...
// OpticalFlow::Bicubic;
interpolation_type i_type = BICUBIC;

noisemodel n_model = LAPLACIAN;
...
// Input arguments:
//     Im1, Im2:						frame 1 and frame
//     2
//	warpIm2:						the warped frame 2 according
//to the current flow field u and v
//	u,v:									the current
//flow field, NOTICE that they are also output arguments
//
//--------------------------------------------------------------------------------------------------------
...
bool OpticalFlow::LoadOpticalFlow(const char *filename, DImage &flow) {
  ...
  if (foo.loadImage(filename) == false) return false;
  if (!flow.matchDimension(foo)) flow.allocate(foo);
...
bool OpticalFlow::LoadOpticalFlow(ifstream &myfile, DImage &flow) {
  Image<unsigned short int> foo;
  if (foo.loadImage(myfile) == false) return false;
  if (!flow.matchDimension(foo)) flow.allocate(foo);
  for (int i = 0; i < flow.npixels(); i++) {
    flow.data()[i * 2] = (double)foo.data()[i * 2] / 160 - 200;
    flow.data()[i * 2 + 1] = (double)foo.data()[i * 2 + 1] / 160 - 200;
